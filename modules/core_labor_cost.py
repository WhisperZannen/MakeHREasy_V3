...
# 核心逻辑:
#   1. 剥离了原本写在页面里的数据库连接与 SQL 提取逻辑。
#   2. 封装了台账的全局基础排序引擎和财务级汇总引擎。
#   3. 统一管理中英文表头映射字典，供前端随时调用。
# ==============================================================================
...
LEDGER_MAP = {
    ...
    '岗位工资': 'base_salary', '工龄工资': 'seniority_pay', '综合补贴': 'comp_subsidy',
    '岗位绩效浮动补贴': 'perf_float_subsidy',
    '通讯费': 'telecom_subsidy', '其他岗位工资': 'other_base_pay', '实习补贴': 'intern_subsidy',
    '高校毕业生/专家津贴': 'grad_allowance',
    '绩效工资标准(参考)': 'perf_standard', 'KPI得分(参考)': 'kpi_score', '考核绩效': 'eval_perf_pay',
    '提成绩效': 'commission_pay',
    '其他月度绩效': 'other_month_perf', '专项奖(含考勤扣罚)': 'special_award', '年终绩效奖': 'year_end_bonus',
    '其他专项奖': 'other_special_award',
    '工资应发合计': 'gross_salary_total', '养老保险-个人': 'pension_personal', '医疗保险-个人': 'medical_personal',
    '失业保险-个人': 'unemployment_personal', '住房公积金-个人': 'provident_fund_personal',
    ...
    '工伤保险-企业': 'work_injury_company', '生育保险-企业': 'maternity_company',
    '住房公积金-企业': 'provident_fund_company',
    '企业年金-企业': 'annuity_company', '日常用餐': 'meal_daily', '加班用餐': 'meal_ot',
    '员工慰问费': 'welfare_condolence',
    '独生子女补贴': 'welfare_single_child', '员工体检费': 'welfare_health_check', '入职体检': 'welfare_entry_check',
    '其他福利': 'welfare_other', '防暑降温费': 'allowance_heat', '女工劳保费': 'allowance_women',
...
def _get_db_connection():
    # 获取当前核心文件所在目录，向上一级进入根目录，然后进入 database 文件夹锁定 db
...
# ------------------------------------------------------------------------------
# 脏数据静默清洗引擎：修正导入 Excel 时可能产生的时间戳尾缀
# ------------------------------------------------------------------------------
def cleanse_db_timestamps():
    ...
    try:
        ...
        cursor.execute(
            "UPDATE labor_cost_ledger SET cost_month = substr(cost_month, 1, 7) WHERE length(cost_month) > 7")
        conn.commit()
    except Exception:
        pass
    finally:
...
# ------------------------------------------------------------------------------
# 全局基础排序引擎 (专用于前端看板与纯净底表生成，不产生汇总行)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 财务级报表计算引擎：执行物理隔离区块并在底部生成小计与总计行
# ------------------------------------------------------------------------------
